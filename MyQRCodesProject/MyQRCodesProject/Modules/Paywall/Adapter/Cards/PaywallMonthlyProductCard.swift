//
//  PaywallMonthlyProductCard.swift
//  MyQRCodesProject
//
...
struct PaywallMonthlyProductCard: View {
    
    private enum Const {
        static let title: String = "Monthly Plan"
        static let priceSlash: String = "/ month"
        ...
        .background(
            RoundedRectangle(cornerRadius: Const.cardRadius)
                ...
                .setShadow(
                    ...
                    color: selectedPlan == .monthly ? .appAccentPrimary.opacity(0.45) : .black.opacity(0.06)
                )
        )
        .overlay(
            RoundedRectangle(cornerRadius: Const.cardRadius)
                .stroke(selectedPlan == .monthly ? .appAccentPrimary : .appTextBorder, lineWidth: 1)
        )
        .onTapGesture {
